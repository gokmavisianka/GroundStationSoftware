...
# -------------------------------------------------------------------------------------------------------------------- #
    def setup_Tab1(self):
        ...
        self.add_GroupBox("var_name", (177, 270, 200, 30), GroupBox="Graph", borderless=True)
        self.add_Label("var_name", "Birim:", (0, 0, 100, 30), GroupBox="var_name")
        self.add_LineEdit("var_name", "", (100, 0, 100, 30), GroupBox="var_name")
    ...
    def setup_Tab2(self):
        ...
        self.add_LineEdit("y", "", (480, 26, 100, 30), GroupBox="Geometry")
        self.add_Label("frame_width", "Çerçeve kalınlığı:", (735, 26, 100, 30), GroupBox="Geometry")
        self.add_LineEdit("frame_width", "", (835, 26, 100, 30), GroupBox="Geometry")
        self.add_Label("width", "Genişlik:", (205, 126, 100, 30), GroupBox="Geometry")
        self.add_LineEdit("width", "", (305, 126, 100, 30), GroupBox="Geometry")
        self.add_Label("height", "Yükseklik:", (559, 126, 100, 30), GroupBox="Geometry")
        self.add_LineEdit("height", "", (659, 126, 100, 30), GroupBox="Geometry")
    ...
    def setup_Tab3(self):
        ...
        self.add_LineEdit("scale_lines_count", "", (277, 70, 100, 30), GroupBox="Scaling")
        ...
        text_ScalingInfo = "*Ölçeklendirme: Grafiğin hangi değer aralığında (minimum değer, maksimum değer) çizileceği ve bu değer aralığının kaça (ayraç sayısı) ayrılacağı ile ilgili değişiklikler\n bu sekmeden yapılır." \
                            " 'Minimum Değer' ve 'Maksimum Değer' ondalıklı veya tam sayı olabilir fakat 'Ayraç Sayısı' ancak tam sayı olarak girilebilir."
    ...
    def setup_Tab4(self):
        ...
        self.add_Label("row", "İndeks:", (380, 26, 100, 30), GroupBox="File")
        self.add_LineEdit("row", "", (480, 26, 100, 30), GroupBox="File")
    ...
    def setup_Tab5(self):
        ...
        self.add_Label("color_minimum_value:", "Min. D. Rengi:", (26, 170, 100, 30), GroupBox="Colors")
        self.add_LineEdit("color_minimum_value", "", (126, 170, 100, 30), GroupBox="Colors")
        self.add_Label("color_average_value", "Ort. D. Rengi:", (380, 170, 100, 30), GroupBox="Colors")
        self.add_LineEdit("color_average_value", "", (480, 170, 100, 30), GroupBox="Colors")
        self.add_Label("color_maximum_value", "Maks. D. Rengi:", (735, 170, 100, 30), GroupBox="Colors")
        self.add_LineEdit("color_maximum_value", "", (835, 170, 100, 30), GroupBox="Colors")
        self.add_Label("color_graph_name", "Grafik Adı Rengi:", (203, 270, 100, 30), GroupBox="Colors")
        self.add_LineEdit("color_graph_name", "", (303, 270, 100, 30), GroupBox="Colors")
        self.add_Label("color_symbol", "Birim Rengi:", (557, 270, 100, 30), GroupBox="Colors")
        self.add_LineEdit("color_symbol", "", (657, 270, 100, 30), GroupBox="Colors")
...
# -------------------------------------------------------------------------------------------------------------------- #
    def setup_Templates(self):
        ...
        self.default_BarGraph = ["GraphType|Bar Grafiği", "graph_name|Mesafe Ölçer", "var_name|cm", "x|150", "y|150", "width|100",
                                 "height|400", "frame_width|24", "minimum_value|0", "maximum_value|100", "scale_lines_count|10",
                                 "bar_color|kırmızı", "frame_color|mavi", "color_maximum_value|turkuaz", "color_average_value|sarı", "color_minimum_value|yeşil", "color_symbol|beyaz", "color_graph_name|kırmızı",
                                 "file_directory|", "row|0", "split_by|:",
        ...
        self.BarGraphProperties = {"GraphType": "Bar Grafiği", "graph_name": None, "var_name": None, "x": None, "y": None, "width": None,
                                   "height": None, "frame_width": None, "minimum_value": None, "maximum_value": None, "scale_lines_count": None,
                                   "bar_color": None, "frame_color": None, "color_maximum_value": None, "color_average_value": None,
                                   "color_minimum_value": None, "color_symbol": None, "color_graph_name": None,
                                   "file_directory": None, "row": None, "split_by": None,
...
class Bar:
    def __init__(self, args):
        ...
        bar_name, var_name, x, y, width, height, fw, minv, maxv, scale_lines_count, bar_color,\
        frame_color, color_maxv, color_avgv, color_minv, color_graph_name, color_var_symbol,\
        file_directory, row, split_by, show_minv, show_avgv, show_maxv, show_minl, show_avgl, show_maxl= args
        x, y, width, height, fw, minv, maxv, scale_lines_count = int(x), int(y), int(width), int(height), int(fw), int(minv), int(maxv), int(scale_lines_count)
        show_minv, show_minl, show_avgv, show_avgl, show_maxv, show_maxl = int(show_minv), int(show_minl), int(show_avgv), int(show_avgl), int(show_maxv), int(show_maxl)
        bar_color, frame_color, color_maxv, color_avgv = colors[bar_color], colors[frame_color], colors[color_maxv], colors[color_avgv]
        color_minv, color_graph_name, color_var_symbol = colors[color_minv], colors[color_graph_name], colors[color_var_symbol]
        self.variable = None
        self.min, self.max, self.avg, self.total, self.num_of_elements = 0, 0, 0, 0, 0
        self.line_number = 0
        self.initial_time = time.perf_counter()
        self.prev_s = 0
        self.bar_name, self.var_name = bar_name, var_name
        self.x, self.y, self.width, self.height = x, y, width, height
        self.fw, self.minv, self.maxv, self.scale_lines_count = fw, minv, maxv, scale_lines_count
        self.bar_color, self.frame_color = bar_color, frame_color
        self.color_minv = color_minv
        self.color_avgv = color_avgv
        self.color_maxv = color_maxv
        self.show_maxv, self.show_maxl, self.show_avgv, self.show_avgl, self.show_minv, self.show_minl = show_maxv, show_maxl, show_avgv, show_avgl, show_minv, show_minl
        self.color_graph_name = color_graph_name
        self.color_var_symbol = color_var_symbol
        self.var_name = var_name
        self.file_directory, self.row, self.split_by = file_directory, row, split_by
        self.lines_count, self.old_lines_count = 0, 0
        Graphs.dictionary.update({bar_name: self})
        self.check()
    ...
    def check2(self):
        ...
        if self.lines_count > self.old_lines_count:
            ...
            self.variable = ((lines[self.lines_count]).split(self.split_by))[self.row]
    ...
    def check(self):
        self.variable = random.randint(self.minv, self.maxv)
        if self.variable < self.min: self.min = self.variable
        if self.variable > self.max: self.max = self.variable
        self.total += self.variable
        self.num_of_elements += 1
    ...
    def draw_frame(self):
        pygame.draw.rect(VP.screen, self.frame_color, [self.x - self.fw // 2, self.y - self.fw // 2, self.width + self.fw, self.height + self.fw])  # Frame
        pygame.draw.rect(VP.screen, VP.background_color, [self.x, self.y, self.width, self.height])  # Background

    def draw_bar(self):
        pos_height = int(self.height * self.variable / (self.maxv - self.minv))
        pygame.draw.rect(VP.screen, self.bar_color, [self.x, self.y + self.height - pos_height, self.width, pos_height])  # The bar itself

    def draw_scale_lines(self):
        value = self.maxv // self.scale_lines_count
        position_y = self.height // self.scale_lines_count
        for n in range(self.scale_lines_count + 1):
            text = self.font.render(f"-{self.maxv - n * value}", True, (255, 255, 255))
            VP.screen.blit(text, (self.x + self.width - 3, self.y + n * position_y - 18))

    def show_values_on_graph(self):
        if self.show_maxv:
            pos_max = int(self.height * self.max / (self.maxv - self.minv)) + 3
            pygame.draw.rect(VP.screen, self.color_minv, [self.x, self.y + self.height - pos_max, self.width, 5])  # Max
            text_max = self.font.render(f"max", True, self.color_minv)
            VP.screen.blit(text_max, (self.x - 75, self.y + self.height - pos_max - 15))
        if self.show_avgv:
            pos_avg = int(self.height * self.avg / (self.maxv - self.minv)) + 3
            pygame.draw.rect(VP.screen, self.color_avgv, [self.x, self.y + self.height - pos_avg, self.width, 5])  # Average
            text_avg = self.font.render(f"Ort", True, self.color_avgv)
            VP.screen.blit(text_avg, (self.x - 75, self.y + self.height - pos_avg - 15))
        if self.show_minv:
            pos_min = int(self.height * self.min / (self.maxv - self.minv)) + 3
            pygame.draw.rect(VP.screen, self.color_maxv, [self.x, self.y + self.height - pos_min, self.width, 5])  # Min
            text_min = self.font.render(f"min", True, self.color_maxv)
            VP.screen.blit(text_min, (self.x - 75, self.y + self.height - pos_min - 15))

    def show_values_as_text(self):
        x, y = self.x - 10, self.y + self.height + 15
        if self.show_maxl:
            text_var_max = self.font.render(f"Max: {self.max}", True, self.color_minv)
            VP.screen.blit(text_var_max, (x, y))
        if self.show_avgl:
            text_var_avg = self.font.render(f"Ort: {round(self.avg, 2)}", True, self.color_avgv)
            VP.screen.blit(text_var_avg, (x, y + 25))
        if self.show_minl:
            text_var_min = self.font.render(f"Min: {self.min}", True, self.color_maxv)
            VP.screen.blit(text_var_min, (x, y + 50))
        text_var_current = self.font.render(f"{self.var_name}: {self.variable}", True, self.color_var_symbol)
        text_bar_name = self.font.render(self.bar_name, True, self.color_graph_name)
        VP.screen.blit(text_var_current, (x, y + 75))
        VP.screen.blit(text_bar_name, (x, y + 100))
